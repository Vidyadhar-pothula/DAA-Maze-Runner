...
class GreedyAI {
    constructor(startNode, goalNode) {
        ...
        this.frontier.put(startNode, 0);
        this.cameFrom = new Map();
        this.cameFrom.set(startNode, null);
    ...
    chooseMove(maze) {
        ...
        for (let { node, cost } of neighbors) {
            if (this.path.length > 1 && node === this.path[this.path.length - 2]) continue;

            let h = this.heuristic(node, this.goalNode);
            if (h < minH) {
                minH = h;
                best = { node, cost };
            }
